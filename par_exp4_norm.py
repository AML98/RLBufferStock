...
from buffer_stock_env_norm import BufferStockEnv
...
from dp_agent_norm import DPAgent
from td3_agent import TD3Agent
from ddpg_agent_norm import DDPGAgent
...
rho = 1.0
beta = 0.96
testing_episodes = 10_000  # Number of episodes to test
...
for param, (param_values, param_name) in params.items():
    ...
    for i, param_value in enumerate(param_values):
        ...
        model.solve()
        dp_baseline_time = elapsed(dp_baseline_tik)
        print(f'DP baseline solution found in {dp_baseline_time}')
        ...
        model_shock.solve()
        dp_shock_time = elapsed(dp_shock_tik)
        print(f'DP shock solution found in {dp_shock_time}')
        ...
        rl_baseline_time = elapsed(rl_baseline_tik)
        print(f'RL baseline training done in {rl_baseline_time}') 
        ...
        baseline_sim_time = elapsed(baseline_sim_tik)
        print(f'Baseline life-cycle simulation done in {baseline_sim_time}')
        ...
        print(f'Baseline expected value: {model.sol.v[0,55]:.2f}')
